...
**跨引擎平滑鲁棒性**:
The phenomenon where a smoothness mechanism preserves behavior-level smoothness metrics (joint acceleration, action jitter) when a policy is replayed in a different physics engine (Isaac → MuJoCo), rather than exhibiting large-magnitude smoothness degradation. Under the current repo evidence, the Jacobian-based `local_sensitivity` constraint + `PID-Lagrangian` path confers this property (Isaac→MuJoCo `joint_acceleration_l2_mean` inflation ~1.08×), as does the heuristic action-rate penalty (~1.01×), while non-Jacobian replacement mechanisms do not: LayerNorm actor (~3.5×), Action Scaling (~12.7×, all seeds collapsed in MuJoCo), and Output Scaling (~4.1×, all seeds collapsed in MuJoCo). This is the core thesis of the current paper-direction work.
_Avoid_: 单引擎内指标优势, 忽略跨引擎行为退化, 把 Isaac-only 平滑当最终结论

**跨引擎退化因子**:
The ratio of a behavior-level smoothness metric measured in MuJoCo versus Isaac for the same policy checkpoint, used to quantify **跨引擎平滑鲁棒性**. A factor near 1.0 indicates the smoothness mechanism transfers across engines; a factor substantially above 1.0 indicates the smoothness observed in Isaac is partially simulator-specific and does not survive cross-engine replay.
_Avoid_: 只看绝对 MuJoCo 数值, 只看 Isaac 绝对数值, 跨引擎比较时忽略退化倍数

**架构级平滑优化线**:
A post-mainline diagnostic branch that tests whether an actor-side architectural constraint can replace the current Jacobian-penalty path without expanding the task definition. Even after both the current **各向异性约束形状线** and **动作时间变化率硬约束线** are closed negatively, the next truly different mechanism line should still remain inside the repo's current same-question low-level smoothness frame before any move into broader frontier directions such as `SysID`, `Residual RL`, perception, or `VLA`. Under the current decision tree, that next mechanism should no longer be framed as another nearby hard-constraint target plus dual-multiplier variant; it should change the mechanism form itself rather than merely reopen another hard-constraint family. Within the repo's current terminology, this **架构级平滑优化线** is the preferred next truly different mechanism line once the current hard-constraint shape and object families have both closed. Its first entry may not be a quiet reopening of the current `SN` family under a new label; it must start from an actor-side mechanism that is not already covered by the repo's current same-question negative boundary for `SN`. That first entry must also replace the current `Jacobian penalty / double backward` path as the active training-side smoothness mechanism rather than remain a Jacobian-supported actor-side variant. It should follow a **任务稳定化配方** opening: inherit the current `SC-PPO 3.8 + PID-Lagrangian` task-valid scaffold, preserve **完全替换对比**, and let the new actor-side mechanism be the sole active smoothness mechanism rather than a `Jacobian + new mechanism` hybrid or a `no-Jacobian / no-heuristic` cold-start. Under the current glossary, methods such as `Adaptive Action Scaling` do not count as this line's first-entry candidate because they are not actor-side architectural constraints. Under the current repo terminology, once `SN` is excluded by the frozen negative boundary and `Adaptive Action Scaling` is excluded by the glossary, the repo opened **正交低增益 actor 线** as a bounded first-entry candidate: orthogonal parametrization on the actor linear layers with a fixed bounded actor output gain, opened as a task-stabilized full-replacement comparison without the Jacobian/double-backward smoothness path. That candidate has now completed and closed negatively under the canonical rough-terrain `11 / 17 / 23`, `512 envs x 400 iterations` entry. The follow-up gain-isolation probe that raises `output_gain` from `0.50` to `1.00` also remains fully collapsed: all three seeds still end at `fall_rate = 1.0`, all three seeds are marked `all_checkpoints_collapsed`, and the higher-gain variant trades into materially larger `action_jitter_l2_mean` and `joint_acceleration_l2_mean` without rescuing task-valid survival. Under the current repo evidence, this rules out the “only low gain caused the failure” explanation and closes the broader orthogonal-actor first-entry candidate. The repo then opened **LayerNorm actor 线** as the next bounded first-entry candidate for **架构级平滑优化线**: actor hidden-layer activation normalization with the Jacobian/double-backward path removed from the active smoothness mechanism, first launched as a task-stabilized full-replacement comparison with `actor_output_gain = 1.00` to avoid reopening the low-gain confound. That first canonical LayerNorm recipe completed under the canonical rough-terrain `11 / 17 / 23`, `512 envs x 400 iterations` entry and did not promote: selected-checkpoint aggregate `fall_rate_mean = 0.8667`, `seed11` remained `all_checkpoints_collapsed`, and although `seed17/23` partially rescued survival, the final-checkpoint aggregate still sat at `fall_rate_mean = 0.7000` with materially elevated `joint_acceleration_l2_mean`, `action_jitter_l2_mean`, and near-saturated late-stage `constraint_violation_rate` on the stronger seeds. The repo then ran the bounded `actor_output_gain = 0.75` repair under the same canonical entry. That repair completed with selected checkpoints `400 / 300 / 400`, all three seeds `selected`, selected-checkpoint aggregate `fall_rate_mean = 0.2000`, and final-checkpoint aggregate `fall_rate_mean = 0.2667`. Under the current three-seed-parallel rule, this is enough to promote **LayerNorm actor 线** to **正式候选线** directly because all three final checkpoints are non-collapsed and remain within the current task floor. However, it still does not win the later Isaac-side internal challenge: `seed17` still relies on `selected checkpoint = 300` while `final checkpoint = 400`, so the line has not yet removed checkpoint-sweep dependence at the final checkpoint. The first two bounded reliability-only follow-ups are now also closed negatively. Holding the recipe fixed but replacing the adaptive PPO schedule with a fixed `1e-5` learning rate fully collapsed task acquisition on all three seeds, with selected checkpoints recorded only for analysis as `200 / 100 / 200` and both selected and final aggregates stuck at `fall_rate_mean = 1.0`. Keeping the adaptive schedule but lowering `policy.init_noise_std` from `1.0` to `0.5` also fully collapsed the same canonical `11 / 17 / 23` entry: all three seeds remained `all_checkpoints_collapsed`, all selected checkpoints regressed to analysis-only `100 / 100 / 100`, and training never entered the late acquisition regime seen in the promoted adaptive baseline. Under the current repo evidence, `output_gain = 0.75` should therefore be read as the first architecture-side `3/3` task-valid survival result, not as a finished smooth replacement for the current `SC-PPO 3.8` mainline, while the local `fixed schedule` and `lower init noise` reliability branches should both be treated as closed negatives rather than as warm near-term retries. The third bounded reliability lever, lowering `desired_kl` from `0.01` to `0.005` while preserving the adaptive schedule, also fully collapsed the same canonical entry: all three seeds ended at `fall_rate = 1.0`, with selected checkpoints regressing to `100 / 0 / 100` and `seed17` recording `selected checkpoint = 0` (every evaluated checkpoint collapsed). The fourth bounded reliability lever, raising `num_learning_epochs` from `2` to `3`, is the only one that cleared the challenge. It completed under the canonical rough-terrain `11 / 17 / 23`, `512 envs x 400 iterations` entry with `selected = final = 400` on all three seeds, aggregate `fall_rate_mean = 0.0167`, `velocity_tracking_error_mean_mean = 0.5378`, `joint_acceleration_l2_mean_mean = 171.9768`, `action_jitter_l2_mean_mean = 0.5193`, and `episode_return_mean_mean = 109.7040`. All three seeds show monotonic late-stage improvement from `cp300` to `cp400` with no degradation. This is the first architecture-side candidate to pass the full Isaac-side internal challenge (`selected checkpoint = final checkpoint = 400` on `3/3` seeds). Relative to the current `SC-PPO 3.8` mainline, this candidate wins on task performance (`fall_rate` 0.017 vs 0.10, `velocity_tracking_error` 0.538 vs 0.641, `episode_return` 109.7 vs 100.3) but loses on smoothness (`joint_acceleration_l2` 172.0 vs 115.9, `action_jitter_l2` 0.519 vs 0.221). This is a task-vs-smoothness trade-off result rather than a clean smoothness replacement. Under the current promotion rules, the line has earned the right to spend `MuJoCo关键两组终验` budget; That cross-engine replay has now completed: `MuJoCo isaac_mainline` aggregate over the three seeds yields `fall_rate_mean = 0.0000`, `velocity_tracking_error_mean_mean = 0.4467`, `joint_acceleration_l2_mean_mean = 602.5776`, `action_jitter_l2_mean_mean = 3.3285`, and `episode_return_mean_mean = -543.5494`. While the candidate does not fall and tracks velocity adequately, joint acceleration is ~5× worse than `SC-PPO 3.8` in MuJoCo (`602.6` vs `125.5`) and action jitter is ~14× worse (`3.33` vs `0.23`). The Isaac→MuJoCo smoothness inflation factor is ~3.5× for this candidate versus ~1.08× for `SC-PPO 3.8`, providing strong evidence that the Jacobian/double-backward constraint path confers significant cross-engine smoothness robustness that the LayerNorm architectural constraint does not replicate.
...
- The canonical `output_gain = 1.00` **LayerNorm actor 线** entry did not promote and left the line open only as a bounded lower-gain repair candidate
- The bounded `output_gain = 0.75` repair then completed the canonical rough-terrain `11 / 17 / 23`, `512 envs x 400 iterations` entry with selected checkpoints `400 / 300 / 400`; under the current `3/3` entry rule this promotes **LayerNorm actor 线** to **正式候选线**, but it still fails the later Isaac-side internal challenge because `seed17` does not satisfy `selected checkpoint = final checkpoint`
- Four bounded `LayerNorm actor output_gain = 0.75` reliability levers were tested under the canonical entry: `fixed schedule` (collapsed), `lower init noise` (collapsed), `lower desired_kl = 0.005` (collapsed, `seed17` selected checkpoint 0), and `num_learning_epochs = 3` (the only lever that passed, with `selected = final = 400` on all three seeds)
- The `num_learning_epochs = 3` candidate is the first architecture-side result to pass the full Isaac-side internal challenge (`3/3` seeds with `selected checkpoint = final checkpoint = 400`), but it trades away smoothness for task performance rather than serving as a clean smoothness replacement for `SC-PPO 3.8`; its completed `MuJoCo isaac_mainline` replay shows ~5× worse joint acceleration and ~14× worse action jitter than `SC-PPO 3.8`, confirming that the Jacobian constraint path provides cross-engine smoothness robustness that LayerNorm does not replicate
- The LayerNorm actor MuJoCo result is the first direct measurement of **跨引擎退化因子** for a non-Jacobian mechanism (~3.5× for `joint_acceleration_l2_mean`), providing quantitative evidence for the paper's core thesis that Jacobian-based constraint confers **跨引擎平滑鲁棒性**
- Completed `MuJoCo isaac_mainline` replay for Action Scaling (~12.7× degradation, all seeds collapsed) and Output Scaling (~4.1× degradation, all seeds collapsed) confirms that all three tested non-Jacobian replacement mechanisms exhibit severe cross-engine smoothness degradation, while both SC-PPO 3.8 (~1.08×) and the heuristic baseline (~1.01×) preserve smoothness across engines
- The full cross-engine degradation table across five methods now forms the quantitative backbone of the paper's core claim: Jacobian-based constraint provides unique cross-engine smoothness robustness that alternative smoothness mechanisms cannot replicate
...
- “既然 **LayerNorm actor 线** 不再像 orthogonal actor 一样全 collapse，是否应直接把它视为新的 live architecture solution 或扩成局部 sweep” was ambiguous between a successful mechanism replacement and a bounded repair opening — resolved: no; first keep the line open only as a bounded repair candidate, with the next minimal follow-up being a lower `actor_output_gain` such as `0.75` rather than a broader architecture neighborhood sweep
- “`output_gain = 0.75` 的 bounded LayerNorm repair 在真实 `11 / 17 / 23`、rough-terrain `512 envs x 400 iterations` 下跑完后，是否已经足够让 **LayerNorm actor 线** 直接升格为 **正式候选线**” was ambiguous between keeping it as a local repair-only result and reusing the repo's shared direct-promotion rule — resolved: yes; the `0.75` repair clears the current `3/3` entry gate and therefore becomes a **正式候选线** directly
- “`output_gain = 0.75` repair 升格后，是否已经赢下后续 Isaac 内部挑战并摆脱对 `checkpoint sweep` 的依赖” was ambiguous between entry-gate promotion and the stricter final-checkpoint-reliability challenge — resolved: no; `seed17` still has `selected checkpoint = 300` while `final checkpoint = 400`, so `final checkpoint` credibility remains unresolved
