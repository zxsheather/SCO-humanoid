...
- The `num_learning_epochs = 3` candidate is the first architecture-side result to pass the full Isaac-side internal challenge (`3/3` seeds with `selected checkpoint = final checkpoint = 400`), but it trades away smoothness for task performance rather than serving as a clean smoothness replacement for `SC-PPO 3.8`
