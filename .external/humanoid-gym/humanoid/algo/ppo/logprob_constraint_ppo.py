...
class LogProbConstraintPPO(PPO):
    """PPO with a hard constraint on ||grad_obs log pi(a_batch | obs_batch)||^2."""
    ...
    def __init__(self, actor_critic, constraint=None, device="cpu", **kwargs):
        ...
        self.constraint_threshold = float(cfg.get("threshold", 6.0))
        self.constraint_subsample_obs = self._parse_constraint_subsample_obs(cfg.get("subsample_obs", 64))
        ...
        self.constraint_epsilon = float(cfg.get("epsilon", 1e-12))

        self.lagrange_multiplier = torch.tensor(float(cfg.get("lambda_init", 0.0)), device=self.device)
    ...
    def _sample_constraint_batch(self, obs_batch, actions_batch):
    ...
    def _logprob_gradient_metrics(self, obs_batch, actions_batch):
        sampled_obs, sampled_actions = self._sample_constraint_batch(obs_batch, actions_batch)
        sampled_obs = sampled_obs.detach().clone().requires_grad_(True)
        sampled_actions = sampled_actions.detach()

        self.actor_critic.update_distribution(sampled_obs)
        ...
        grad_norm = torch.sqrt(torch.clamp(squared_norm, min=self.constraint_epsilon))

        cost_mean = squared_norm.mean()
        cost_max = squared_norm.max()
        cost_quantile = torch.quantile(squared_norm, self.constraint_cost_quantile)
        cost_for_update = self._aggregate_constraint_cost(cost_mean, cost_max, cost_quantile)
        violation_rate = torch.mean((squared_norm > self.constraint_threshold).float())
        return {
            "cost_mean": cost_mean,
            "cost_max": cost_max,
    ...
    def _integrate_constraint_error(self, error):
        ...
        lower_bound_active = float(self.lagrange_multiplier.item()) <= (
            self.constraint_lambda_min + self.constraint_epsilon
        )
    ...
    def update(self):
        ...
        mean_logprob_gradient_norm = 0
        mean_logprob_gradient_norm_max = 0
        ...
        for (
            ...
            logprob_gradient_norm_mean = torch.tensor(0.0, device=self.device)
            logprob_gradient_norm_max = torch.tensor(0.0, device=self.device)
            constraint_sample_count = 0
            if self.constraint_enabled:
                constraint_stats = self._logprob_gradient_metrics(obs_batch, actions_batch)
                ...
                logprob_gradient_norm_mean = constraint_stats["grad_norm_mean"]
                logprob_gradient_norm_max = constraint_stats["grad_norm_max"]
            ...
            mean_logprob_gradient_norm += logprob_gradient_norm_mean.item()
            mean_logprob_gradient_norm_max += logprob_gradient_norm_max.item()
        ...
        mean_logprob_gradient_norm /= num_updates
        mean_logprob_gradient_norm_max /= num_updates

        lagrange_delta = 0.0
        if self.constraint_enabled:
            lagrange_delta = self._update_lagrange_multiplier(
                mean_constraint_cost_update - self.constraint_threshold
            )
        ...
        trace_entry = {
            ...
            "logprob_gradient_cost_mean": float(mean_constraint_cost),
            "logprob_gradient_cost_update": float(mean_constraint_cost_update),
            "logprob_gradient_cost_max": float(mean_constraint_cost_max),
            "logprob_gradient_cost_quantile": float(mean_constraint_cost_quantile),
            "logprob_gradient_norm_mean": float(mean_logprob_gradient_norm),
            "logprob_gradient_norm_max": float(mean_logprob_gradient_norm_max),
    ...
    def get_logging_stats(self):
        ...
        return {
            ...
            "Constraint/logprob_gradient_cost_mean": self.latest_stats["logprob_gradient_cost_mean"],
            "Constraint/logprob_gradient_cost_update": self.latest_stats["logprob_gradient_cost_update"],
            "Constraint/logprob_gradient_cost_max": self.latest_stats["logprob_gradient_cost_max"],
            "Constraint/logprob_gradient_cost_quantile": self.latest_stats["logprob_gradient_cost_quantile"],
            "Constraint/logprob_gradient_norm_mean": self.latest_stats["logprob_gradient_norm_mean"],
            "Constraint/logprob_gradient_norm_max": self.latest_stats["logprob_gradient_norm_max"],
    ...
    def load_extra_state_dict(self, state_dict):
        if not state_dict:
            return
        with torch.no_grad():
            self.lagrange_multiplier.fill_(
                float(state_dict.get("lagrange_multiplier", self.lagrange_multiplier.item()))
            )
    ...
    def get_artifact_payload(self):
        if not self.constraint_trace:
            return {}
        cost_history = [entry["logprob_gradient_cost_mean"] for entry in self.constraint_trace]
        return {
            "constraint_metrics": {
                "constraint_source": "logprob_gradient_hard_constraint",
                "constraint_penalty_form": "mean(||grad_obs log pi(a_batch | obs_batch)||^2)",
                ...
                "pid_integral_mode": self.constraint_pid_integral_mode,
                "pid_integral_decay": float(self.constraint_pid_integral_decay),
                "logprob_gradient_cost_mean": self.latest_stats.get("logprob_gradient_cost_mean"),
                "logprob_gradient_cost_update": self.latest_stats.get("logprob_gradient_cost_update"),
                "logprob_gradient_cost_max": self.latest_stats.get("logprob_gradient_cost_max"),
                "logprob_gradient_cost_quantile": self.latest_stats.get("logprob_gradient_cost_quantile"),
                "logprob_gradient_cost_std": statistics.pstdev(cost_history) if len(cost_history) > 1 else 0.0,
                "logprob_gradient_norm_mean": self.latest_stats.get("logprob_gradient_norm_mean"),
                "logprob_gradient_norm_max": self.latest_stats.get("logprob_gradient_norm_max"),
            },
