...
CONFIG="${CONFIG:-configs/methods/omnisafe_ppolag_cost_bridge_smoke.json}"
RUN_NAME=""
ARTIFACTS_ROOT=""
ORIGINAL_ARGS=("$@")
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/run_omnisafe_cost_bridge_smoke.py" --config="${CONFIG}" "${ORIGINAL_ARGS[@]}"
status=$?
if [[ ${status} -eq 0 ]]; then
  exit 0
fi
...
artifact = artifacts_root / run_name / "omnisafe_cost_bridge_smoke.json"
...
if [[ "${artifact_status}" == "complete" ]]; then
  echo "Recovered non-zero Isaac exit (${status}) because cost-bridge smoke artifact is complete." >&2
  exit 0
fi
