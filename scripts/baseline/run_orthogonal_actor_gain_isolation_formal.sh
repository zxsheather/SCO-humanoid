...
SWEEP_CONFIG="${SWEEP_CONFIG:-${REPO_ROOT}/configs/sweeps/rough_terrain_orthogonal_actor_gain_isolation_probe.json}"
...
if [[ ! -x "${CXX_BIN}" ]]; then
  echo "c++ compiler not executable: ${CXX_BIN}" >&2
  exit 1
fi

if [[ ! -x "${CC_BIN}" ]]; then
  echo "cc compiler not executable: ${CC_BIN}" >&2
  exit 1
fi
