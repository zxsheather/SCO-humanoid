...
REPO_ROOT = repo_root()
DEFAULT_CONFIG = REPO_ROOT / "configs" / "methods" / "output_scaling_threshold_38_quantile_090_pid_lower_bound_clamp.json"
...
DEFAULT_ANALYSIS_ROOT = REPO_ROOT / "artifacts" / "analysis" / "output_scaling_diagnostic"
...
def collect_summary(config: dict[str, Any], run_name: str, args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "diagnostic": "output_scaling_replacement_mechanism_feasibility",
        "scope": "输出缩放替代机制线",
        "run_name": run_name,
        "preset": args.preset,
        ...
        "interpretation_boundary": [
            "This diagnostic checks whether the first canonical output-scaling recipe trains and emits interpretable artifacts.",
            "It must not be read as a formal mainline challenge result before the three-seed entry gate is run.",
            "It keeps the shared local-sensitivity evaluation readout, but output scaling is the sole active smoothness mechanism.",
        ],
    }
...
def print_plan(config: dict[str, Any], config_path: Path, run_name: str, args: argparse.Namespace) -> None:
    print(f"Output-scaling diagnostic preset: {args.preset}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run a bounded output-scaling replacement-mechanism diagnostic.")
    parser.add_argument("--config", default=None, help="Path to the output-scaling method config JSON.")
