...
REPO_ROOT = repo_root()
DEFAULT_CONFIG = REPO_ROOT / "configs" / "methods" / "layernorm_actor_output_gain_1000_formal_probe.json"
...
DEFAULT_ANALYSIS_ROOT = REPO_ROOT / "artifacts" / "analysis" / "layernorm_actor_diagnostic"
...
def run_name_for(config: dict, args: argparse.Namespace) -> str:
    if args.run_name:
...
def train_manifest_path(config: dict, run_name: str) -> Path:
    return artifact_dir(config, run_name) / "manifest.json"


def eval_summary_path(config: dict, run_name: str) -> Path:
    return artifact_dir(config, run_name) / "checkpoint_sweep_summary.json"


def train_complete(config: dict, run_name: str) -> bool:
...
def evaluate_complete(config: dict, run_name: str) -> bool:
    return eval_summary_path(config, run_name).exists()


def resolve_load_run(config: dict, run_name: str) -> str:
    manifest_path = train_manifest_path(config, run_name)
...
def latest_checkpoint_id(config: dict, run_name: str) -> int | None:
    manifest_path = train_manifest_path(config, run_name)
    if not manifest_path.exists():
        return None
    manifest = read_json(manifest_path)
...
def build_evaluate_command(config: dict, config_path: Path, run_name: str, args: argparse.Namespace) -> list[str]:
    command = [
        sys.executable,
        str(EVAL_SCRIPT),
        f"--config={relative_to_repo(config_path)}",
...
def collect_summary(config: dict, run_name: str, args: argparse.Namespace) -> dict:
    summary_path = eval_summary_path(config, run_name)
    manifest_path = train_manifest_path(config, run_name)
    payload: dict = {
        "diagnostic": "layernorm_actor_architecture_replacement_mechanism_feasibility",
        "scope": "架构级平滑优化线",
        "run_name": run_name,
        "preset": args.preset,
        ...
        "interpretation_boundary": [
            "This diagnostic checks whether the first LayerNorm actor recipe trains and emits interpretable artifacts.",
            "It must not be read as a formal mainline challenge result before the three-seed entry gate is run.",
            "It keeps the shared local-sensitivity evaluation readout, while actor-side LayerNorm is the sole active smoothness mechanism.",
        ],
    }
...
def write_analysis_summary(config: dict, run_name: str, args: argparse.Namespace) -> Path:
...
def print_plan(config: dict, config_path: Path, run_name: str, args: argparse.Namespace) -> None:
    print(f"LayerNorm-actor diagnostic preset: {args.preset}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run a bounded LayerNorm-actor architecture diagnostic.")
    parser.add_argument("--config", default=None, help="Path to the LayerNorm-actor method config JSON.")
    ...
    if args.stage in {"train", "all"}:
        if not (args.skip_completed and train_complete(config, run_name)):
            exit_code = run_command(build_train_command(config_path, run_name, args), cwd=REPO_ROOT, dry_run=args.dry_run)
            if exit_code != 0 and not (args.skip_completed and train_complete(config, run_name)):
                return exit_code
    ...
    if args.stage in {"evaluate", "all"}:
        if not (args.skip_completed and evaluate_complete(config, run_name)):
            ...
            if exit_code != 0 and not evaluate_complete(config, run_name):
                return exit_code
