...
- Statistical audit: paired bootstrap CIs support LCP over SC-PPO on
  Isaac fall, velocity error, return, and sensitivity; the joint-acceleration
  advantage is directional but overlaps zero because seed-level variance is
  large
...
- Read: LCP has the best action jitter and is much stronger than
  ...
  and return
- Mechanism read: this is a control-path metric split, not a contradiction.
  LCP is the cleanest policy-output/action-jitter row, while MuJoCo joint
  acceleration and return also depend on closed-loop tracking, PD response,
  contact timing, and simulator-specific dynamics. The leave-one-seed audit
  preserves the split: LCP remains best on aggregate action jitter, and the
  revised heuristic remains best on aggregate joint acceleration and return.
  Source note: `docs/full-paper/mujoco-mixed-evidence-mechanism.md`
- Statistical audit: LCP over SC-PPO on MuJoCo action jitter has a paired
  bootstrap interval excluding zero; LCP-vs-heuristic MuJoCo joint
  acceleration and return remain metric-dependent/descriptive rather than
...
### 5.4b Selected-vs-final checkpoint robustness

[T0e: selected-vs-final checkpoint audit; generated as
`docs/full-paper/selected-vs-final-checkpoint-robustness.md`]

- LCP is close to final-only behavior: only seed `11` selects `300`
  instead of final checkpoint `400`, and aggregate final-vs-selected deltas
  are small
- SC-PPO is dynamic-selection-sensitive: final checkpoints improve velocity
  and return but worsen joint acceleration and jitter
- Revised heuristic is task-selection-sensitive: final checkpoints improve
  velocity/return slightly but increase fall rate and dynamic roughness
...
- LCP coefficient sensitivity: `0.002` is best in the narrow
  `0.001/0.002/0.004` diagnostic grid on seeds `11/17/23/29/31`
  (T0c:
...
- Actuator low-pass proxy stress: the bounded 50 ms setting produces similar
  jitter factors for all three methods, while the 100 ms setting has high fall
  rates and should be read as a task-validity boundary rather than a ranking
...
- The mixed MuJoCo evidence has a tracked mechanism decomposition, but the
  explanation remains aggregate-level and correlational rather than an
  intervention-level causal proof
- No real-robot validation
- The actuator-bandwidth stress test is a MuJoCo diagnostic with chosen
  50 ms and 100 ms action low-pass constants, not a calibrated actuator or
  hardware result
- MuJoCo `hfield_moderate` is included only as bounded second-setting evidence

### 7.2 Checkpoint dependence
- SC-PPO 3.8 mainline relies on checkpoint-sweep selection
  (five-seed selected: 300/300/400/400/400)
- Selected-vs-final audit is now explicit: SC-PPO selection mainly protects
  dynamic smoothness, while heuristic selection mainly protects task validity
...
- Paired bootstrap uncertainty audit is reported, but it should not be
  written as a large-sample hypothesis test
...
| T0e | Table | Selected-vs-final checkpoint robustness | `docs/full-paper/selected-vs-final-checkpoint-robustness.md`, generated by `scripts/analysis/analyze_checkpoint_robustness.py`; runtime summary in `artifacts/analysis/checkpoint_robustness/summary.json` |
| T0f | Table | Bounded hfield_moderate second-setting replay | `artifacts/analysis/hfield_moderate_second_setting/summary.md`; per-run CSV in `artifacts/analysis/hfield_moderate_second_setting/table_hfield_moderate_second_setting.csv` |
| T0g | Table | Actuator-bandwidth robustness sweep | `artifacts/analysis/actuator_latency_robustness/summary.md`, generated by `scripts/baseline/run_actuator_latency_robustness_sweep.py`; per-run CSV in `artifacts/analysis/actuator_latency_robustness/table_actuator_latency_robustness.csv` |
...
| D4 | Data | Statistical robustness audit for T0/T0b | `docs/full-paper/statistical-robustness-results.md`, generated by `scripts/analysis/analyze_full_paper_statistics.py`; runtime summary in `artifacts/analysis/full_paper_statistics/summary.json` |
| D5 | Data | Matched MuJoCo mixed-evidence mechanism decomposition | `docs/full-paper/mujoco-mixed-evidence-mechanism.md`, generated by `scripts/analysis/analyze_mujoco_mixed_evidence.py`; runtime summary in `artifacts/analysis/mujoco_mixed_evidence/summary.json` |
| D6 | Data | LCP-style baseline fidelity and naming boundary | `docs/full-paper/lcp-style-fidelity-audit.md`; primary config in `configs/methods/lcp_soft_jacobian_penalty_diagnostic.json` |
