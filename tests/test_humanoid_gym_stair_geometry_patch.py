...
import patch_humanoid_gym_stair_geometry_env as stair_patch  # noqa: E402
...
HEIGHT_ONLY_PATCHED_SOURCE = """import os
import numpy as np
...
class HumanoidGymStairGeometryPatchTests(unittest.TestCase):
    def test_patch_terrain_source_inserts_height_and_width_runtime_controls(self) -> None:
        ...
        self.assertIn(_common.STAIR_WIDTH_SCALE_ENV, patched)
        self.assertIn("step_width=0.4 * stair_width_scale", patched)
        self.assertIn("discrete_obstacles_height * stair_height_scale", patched)

    def test_patch_terrain_source_upgrades_height_only_patch(self) -> None:
        patched, changed = stair_patch.patch_terrain_source(HEIGHT_ONLY_PATCHED_SOURCE)
        self.assertTrue(changed)
        self.assertIn(_common.STAIR_HEIGHT_SCALE_ENV, patched)
        self.assertIn(_common.STAIR_WIDTH_SCALE_ENV, patched)
        self.assertIn("step_width=0.4 * stair_width_scale", patched)
    ...
    def test_verify_required_local_patches_rejects_missing_width_patch(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config = {"runtime_env": {_common.STAIR_WIDTH_SCALE_ENV: "2.0"}}
