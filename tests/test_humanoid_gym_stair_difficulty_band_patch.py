...
import patch_humanoid_gym_stair_difficulty_band_env as stair_patch  # noqa: E402


CAP_PATCHED_SOURCE = """import os
import numpy as np
...
class HumanoidGymStairDifficultyBandPatchTests(unittest.TestCase):
    def test_patch_terrain_source_inserts_band_runtime_controls(self) -> None:
        patched, changed = stair_patch.patch_terrain_source(CAP_PATCHED_SOURCE)
        self.assertTrue(changed)
        self.assertIn(_common.STAIR_DIFFICULTY_MIN_ENV, patched)
        self.assertIn(_common.STAIR_DIFFICULTY_MAX_ENV, patched)
        self.assertIn("stair_difficulty_min_raw", patched)
        self.assertIn("np.random.uniform(stair_difficulty_min, stair_difficulty_max)", patched)
    ...
    def test_verify_required_local_patches_rejects_missing_band_patch(self) -> None:
